    ...
    try:
        ...
        if bot:
            row = await bot.log_service.db_fetch_one(
                "SELECT name FROM users WHERE user_id = ?", (user_openid,)
            )
            if row:
...
async def ranking(event, match):
    ...
    if not ranking_list:
        ...
        for idx, item in enumerate(ranking_list):
            ...
            name = await get_display_name(event, item['user_openid'])
            md += f">{medal} {name} —— **{item['correct_count']}题**\n"
